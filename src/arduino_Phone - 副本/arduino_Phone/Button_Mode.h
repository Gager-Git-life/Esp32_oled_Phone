...
extern bool Second_fun_flg,Three_fun_flg,Fourth_fun_flg,Five_fun_flg,Manul_Seting_flg;
...
const int Third_fuc_count[] =  {Network_count,0,0,0,7,0,0,0,3};
...
void IR_datection(int count){
  IR_Recvice();
  if(count == 1){//第一层
    if(Num == 1) Second_fun_flg = true , last_recode_time = millis();
    else if(Num == 2) {
    ...
    if(Num == 1) Three_fun_flg = true, last_recode_time = millis();
    else if(Num == 2) {
      ...
      if(Num == 1){
        ...
        if(Third_demoMode == 0) Third_demoMode = (Third_fuc_count[demoMode]  - 1);
        else Third_demoMode = ( Third_demoMode - 1);
        last_recode_time = millis();
      }
      else if(Num ==3){
        Third_demoMode = (Third_demoMode + 1)%Third_fuc_count[demoMode];
        last_recode_time = millis(); 
      }
  }
  Num = 0;
...
void IO_detection(){
  ...
  //第二层
  else if(Second_fun_flg && !Three_fun_flg && !Fourth_fun_flg){
    if(IR_flg) IR_datection(2);
    if(digitalRead(Previous) == LOW){
      //delay(50);  
      if(digitalRead(Previous) == LOW){
        Sec_demoMode = (Sec_demoMode+1)%Second_fuc_count[demoMode]; 
  ...
  //第四层
    ...
    if(digitalRead(Previous) == LOW){
      //delay(50);
      if(digitalRead(Previous) == LOW){
         Fourth_demoMode = (Fourth_demoMode + 1); 
         last_recode_time = millis(); 
        }
        //delay(150);
    }
      ...
      //delay(50);
      if(digitalRead(Latter) == LOW){
        if(Fourth_demoMode == 0) Fourth_demoMode = 0;
  ...
  //if(digitalRead(REturn)) IO_count += 4;
}
